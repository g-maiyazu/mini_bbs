...
if(!empty($_POST)){
  ...
  if(empty($error)){
    ...
    // アップロード時、tmp_nameに一時的にファイルを保存し、第2引数に移動させる
    move_uploaded_file($_FILES['image']['tmp_name'], '../member_picture/' . $image);

    // POSTデータをSESSIONにつめて画面遷移
    $_SESSION['join'] = $_POST;
...
// 確認画面からの戻る動作
if($_REQUEST['action'] == 'rewrite' && isset($_SESSHION['join'])){
  $_POST = $_SESSION['join'];
}
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <dd>
      ...
      <?php if($error['image'] === 'type'): ?>
        <p class="error">*写真はpng/jpg/gifの形式を指定してください</p>
      <?php endif; ?>
      <?php if(!empty($error)): ?>
